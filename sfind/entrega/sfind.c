...
int processFile(struct stat dir_info, COMMAND command, struct dirent *dp, char *filepath){
...
int traverseDirectory(COMMAND command){
    ...
    while((dp = readdir(dir)) != NULL){
        ...
        processFile(dir_info, command, dp, filepath);
...
void argumentHandling(int argc, char*argv[], COMMAND *command){
    ...
    for(argno = 2; argno < argc; argno++){
        if(strcmp("-name",argv[argno]) == 0 && argno + 1 < argc){
            ...
            argno++;
        }else if(strcmp(";",argv[argno]) && strcmp(argv[argno], "{}")){
